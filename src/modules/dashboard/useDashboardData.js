...
export default function useDashboardData() {
    ...
    const { cards, renewals, marketPrices } = usePortfolio();
    ...
    const investmentSnapshot = useMemo(() => {
        ...
        const sections = [
            { key: "k401", label: "401(k)", enabled: !!financialConfig?.track401k && (holdings.k401?.length > 0), color: T.status.blue },
            { key: "roth", label: "Roth IRA", enabled: !!financialConfig?.trackRothContributions && (holdings.roth?.length > 0), color: T.status.purple },
            { key: "brokerage", label: "Brokerage", enabled: !!financialConfig?.trackBrokerage && (holdings.brokerage?.length > 0), color: T.accent.emerald },
            { key: "hsa", label: "HSA", enabled: !!financialConfig?.trackHSA && (holdings.hsa?.length > 0), color: T.accent.emerald },
            { key: "crypto", label: "Crypto", enabled: !!financialConfig?.trackCrypto && (holdings.crypto?.length > 0), color: T.status.amber },
        ];
        const result = [];
        let grandTotal = 0;
        ...
        if (realAudits.length >= 2) {
            ...
            if (debtValues.length >= 2 && debtValues[0] > 100) {
                ...
                for (let i = 1; i < debtValues.length; i++) {
                    ...
                    if (delta > 0) { totalPaydown += delta; activeIntervals++; }
                }
    ...
    const alerts = useMemo(() => {
        ...
        if (current?.isTest) {
            result.push(
                { icon: "🎯", color: T.status.green, title: "Debt-Free", text: `At $400/wk → ${freedomStats.freeDateStr || "Oct 2026"}` },
        ...
        if (realAudits.length >= 2) {
            ...
            if (financialConfig?.track401k && financialConfig?.k401ContributedYTD > 0 && financialConfig?.taxBracketPercent > 0) {
                ...
                result.push({ icon: "🛡️", color: T.status.purple, title: "Tax Shield", text: `${fmt(taxSaved)} saved at ${financialConfig.taxBracketPercent}% ` });
            }
            ...
            if (nwAudits.length >= 2) {
                ...
                if (Math.abs(delta) > 50) {
                    const up = delta > 0;
                    result.push({ icon: up ? "💰" : "📉", color: up ? T.status.green : T.status.amber, title: "Net Worth", text: `${up ? "+" : ""}${fmt(delta)} over last ${nwAudits.length} audits` });
                }
            }
            if (realAudits.length >= 2) {
                ...
                if (lScore != null && pScore != null && Math.abs(lScore - pScore) >= 3) {
                    ...
                    if (factors.length > 0) {
                        ...
                        result.push({ icon: up ? "⚡" : "🔍", color: up ? T.status.green : T.status.amber, title: `Score ${up ? "+" : ""}${lScore - pScore} `, text: `Driven by ${labels[biggest.name] || biggest.name} ` });
                    }
                }
            }
            ...
            if (currentSavings > 0) {
                ...
                for (const m of milestones) {
                    ...
                    if (gap > 0 && gap <= 500 && currentSavings >= m * 0.5) {
                        ...
                        result.push({ icon: "🏦", color: T.status.green, title: "Almost There", text: `$${Math.round(gap)} away from ${label} saved` });
                        break;
                    }
                }
            }
            ...
            (cards || []).forEach(c => {
                if (c.hasPromoApr && c.promoAprExp) {
                    ...
                    if (daysLeft > 0 && daysLeft <= 60) {
                        ...
                        result.push({ icon: "⏰", color: T.status.amber, title: "Promo Ending", text: `${name} promo expires in ${daysLeft}d`, pulse: daysLeft <= 14 });
                    }
                }
            });
            ...
            if (spendData.length >= 3) {
                ...
                const priorSpends = spendData.slice(0, -1).map(s => s.spent).filter(v => v > 0);
                if (priorSpends.length >= 2 && recentSpend > 0) {
                    ...
                    if (avgPrior > 50 && recentSpend > avgPrior * 1.5) {
                        ...
                        result.push({ icon: "🔥", color: T.status.red, title: "Spending Spike", text: `${pct}% above your ${priorSpends.length}-week average` });
                    }
                }
            }
            ...
            if (realAudits.length >= 2) {
                ...
                if (prevRenTotal > 0 && latestRenTotal > prevRenTotal) {
                    ...
                    if (increase > 25 || pctIncrease > 15) {
                        result.push({
                            icon: "📊", color: T.status.amber, title: "Bills Up",
