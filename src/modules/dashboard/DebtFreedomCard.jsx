 ...
 */
export default function DebtFreedomCard({ cards = [], freedomStats }) {
  const totalDebt = cards.reduce((s, c) => s + (parseFloat(c?.balance) || 0), 0);
  if (totalDebt < 100) return null; // No meaningful debt

  const { freeDateStr, weeklyPaydown } = freedomStats || {};
  const hasProjection = freeDateStr && weeklyPaydown > 0;
  ...
  const getMessage = () => {
    ...
    if (monthsToFree <= 3) return "You're in the final stretch. Stay locked in.";
    if (monthsToFree <= 6) return "Incredible momentum. Half a year away from freedom.";
    if (monthsToFree <= 12) return "Under a year out. Every dollar counts.";
