...
import { Eye, EyeOff, ArrowLeft, Cloud, Download, Upload, CheckCircle, AlertTriangle, ChevronDown, Loader2, ExternalLink, Pencil, Check } from "lucide-react";
...
import { fetchMarketPrices, POPULAR_CRYPTO } from "../marketData.js";
...
export default function SettingsTab({ apiKey, setApiKey, onClear, onFactoryReset,
    useStreaming, setUseStreaming, onBack, onRestoreComplete,
    ...
    const [activeSegment, setActiveSegment] = useState("app"); // "app" | "finance"
    const [appTab, setAppTab] = useState("ai"); // ai | backup | security | system
    const [financeTab, setFinanceTab] = useState("income"); // income | debts | assets | targets | rules
    ...
    return <div className="slide-pane safe-pane-noheader" style={{
        ...
        <div style={{
            ...
            <div style={{ textAlign: "center", flex: 1 }}>
                ...
                <p style={{ fontSize: 10, color: T.text.dim, marginTop: 2, fontFamily: T.font.mono, margin: 0 }}>VERSION 1.3.1-BETA</p>
            </div>
            <div style={{ width: 36 }}></div> {/* Spacer to preserve center alignment */}
        </div>
        {/* Scrollable body */}
        <div className="safe-scroll-body page-body" style={{
            flex: 1,
            ...
            {/* Segmented Control */}
            <div style={{ display: "flex", background: T.bg.elevated, borderRadius: T.radius.md, padding: 4, marginBottom: 20, border: `1px solid ${T.border.default}` }}>
                <button onClick={() => setActiveSegment("app")} style={{
                    flex: 1, padding: "8px 0", borderRadius: T.radius.sm, border: "none",
                    background: activeSegment === "app" ? T.bg.card : "transparent",
                    color: activeSegment === "app" ? T.text.primary : T.text.secondary,
                    fontSize: 12, fontWeight: activeSegment === "app" ? 700 : 600,
                    boxShadow: activeSegment === "app" ? T.shadow.subtle : "none",
                    transition: "all .2s ease", cursor: "pointer"
                }}>App Preferences</button>
                <button onClick={() => setActiveSegment("finance")} style={{
                    flex: 1, padding: "8px 0", borderRadius: T.radius.sm, border: "none",
                    background: activeSegment === "finance" ? T.bg.card : "transparent",
                    color: activeSegment === "finance" ? T.text.primary : T.text.secondary,
                    fontSize: 12, fontWeight: activeSegment === "finance" ? 700 : 600,
                    boxShadow: activeSegment === "finance" ? T.shadow.subtle : "none",
                    transition: "all .2s ease", cursor: "pointer"
                    ...
                    <div style={{ display: "flex", gap: 6, marginBottom: 16, flexWrap: "wrap" }}>
                        {[
                            ...
                            <button key={t.id} onClick={() => setAppTab(t.id)} style={{
                                padding: "6px 10px", borderRadius: 999, border: `1px solid ${appTab === t.id ? T.accent.primary : T.border.default}`,
                                background: appTab === t.id ? T.accent.primaryDim : T.bg.elevated, color: appTab === t.id ? T.accent.primary : T.text.secondary,
            ...
            {/* ── Financial Constants ────────────────────────────────────── */}
            {
                activeSegment === "finance" && (
                    <Card style={{ borderLeft: `3px solid ${T.status.blue}40` }}>
                        <Label>Financial Constants</Label>
                        <div style={{ display: "flex", gap: 6, marginBottom: 16, flexWrap: "wrap" }}>
                            {[
                                { id: "income", label: "Income & Budget" },
                                { id: "debts", label: "Debts & Credit" },
                                { id: "assets", label: "Assets & Goals" },
                                { id: "targets", label: "Targets & Limits" },
                                { id: "rules", label: "Rules & Advanced" },
                            ].map(t => (
                                <button key={t.id} onClick={() => setFinanceTab(t.id)} style={{
                                    padding: "6px 10px", borderRadius: 999, border: `1px solid ${financeTab === t.id ? T.accent.primary : T.border.default}`,
                                    background: financeTab === t.id ? T.accent.primaryDim : T.bg.elevated, color: financeTab === t.id ? T.accent.primary : T.text.secondary,
                                    fontSize: 10, fontWeight: 700, cursor: "pointer", fontFamily: T.font.mono
                                }}>{t.label}</button>
                            ))}
                        </div>
                        ...
                        <div style={{ marginBottom: 24, display: financeTab === "income" ? "block" : "none" }}>
                            <h3 style={{ fontSize: 13, fontWeight: 700, marginBottom: 4, color: T.text.primary, borderBottom: `1px solid ${T.border.subtle}`, paddingBottom: 6 }}>Account Tracking</h3>
                            <p style={{ fontSize: 10, color: T.text.muted, marginBottom: 12, lineHeight: 1.4 }}>Toggle which balances are included in the audit input form. Disabled accounts are hidden from the form and not required to run an audit.</p>
                            {[
                        ...
                        < div style={{ marginBottom: 24, display: financeTab === "income" ? "block" : "none" }
                            ...
                            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", borderBottom: `1px solid ${T.border.subtle}`, paddingBottom: 6, marginTop: 16, marginBottom: 12 }}>
                                <div style={{ flex: 1 }}>
                                    <h3 style={{ fontSize: 13, fontWeight: 700, color: T.text.primary, marginBottom: 2 }}>Additional Income Sources</h3>
                        ...
                        < div style={{ marginBottom: 24, display: financeTab === "income" ? "block" : "none" }}>
                            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", borderBottom: `1px solid ${T.border.subtle}`, paddingBottom: 6, marginBottom: 12 }}>
                                <div style={{ flex: 1 }}>
                                    <h3 style={{ fontSize: 13, fontWeight: 700, color: T.text.primary, marginBottom: 2 }}>Monthly Budget Categories</h3>
                        ...
                        < div style={{ marginBottom: 24, display: financeTab === "debts" ? "block" : "none" }}>
                            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", borderBottom: `1px solid ${T.border.subtle}`, paddingBottom: 6, marginBottom: 12 }}>
                                <div style={{ flex: 1 }}>
                                    <h3 style={{ fontSize: 13, fontWeight: 700, color: T.text.primary, marginBottom: 2 }}>Non-Card Debts</h3>
                            ...
                            {editingSection === "debts" || (financialConfig?.nonCardDebts || []).length === 0 ? <>
                                {(financialConfig?.nonCardDebts || []).map((debt, i) => (
                                    <div key={i} style={{ background: T.bg.elevated, borderRadius: T.radius.md, border: `1px solid ${T.border.default}`, padding: 10, marginBottom: 8 }}>
                                        ...
                                        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 0.5fr", gap: 6 }}>
                                            <div style={{ position: "relative" }}>
                                                ...
                                                <input type="number" inputMode="decimal" pattern="[0-9]*" value={debt.balance || ""} onChange={e => { const arr = [...(financialConfig.nonCardDebts || [])]; arr[i] = { ...arr[i], balance: parseFloat(e.target.value) || 0 }; setFinancialConfig({ ...financialConfig, nonCardDebts: arr }); }}
                                                    placeholder="Balance" style={{ width: "100%", padding: "6px 6px 6px 16px", borderRadius: T.radius.sm, border: `1px solid ${T.border.default}`, background: T.bg.card, color: T.text.primary, fontSize: 10 }} />
                                            </div>
                                            <div style={{ position: "relative" }}>
                                                <span style={{ position: "absolute", left: 6, top: "50%", transform: "translateY(-50%)", color: T.text.dim, fontSize: 10, fontWeight: 600 }}>$</span>
                                                <input type="number" inputMode="decimal" pattern="[0-9]*" value={debt.minimum || ""} onChange={e => { const arr = [...(financialConfig.nonCardDebts || [])]; arr[i] = { ...arr[i], minimum: parseFloat(e.target.value) || 0 }; setFinancialConfig({ ...financialConfig, nonCardDebts: arr }); }}
                                                    placeholder="Min pay" style={{ width: "100%", padding: "6px 6px 6px 16px", borderRadius: T.radius.sm, border: `1px solid ${T.border.default}`, background: T.bg.card, color: T.text.primary, fontSize: 10 }} />
                                            </div>
                                            <div style={{ position: "relative" }}>
                                                <input type="number" inputMode="decimal" pattern="[0-9]*" step="0.01" value={debt.apr || ""} onChange={e => { const arr = [...(financialConfig.nonCardDebts || [])]; arr[i] = { ...arr[i], apr: parseFloat(e.target.value) || 0 }; setFinancialConfig({ ...financialConfig, nonCardDebts: arr }); }}
                                                    placeholder="APR" style={{ width: "100%", padding: "6px 18px 6px 6px", borderRadius: T.radius.sm, border: `1px solid ${T.border.default}`, background: T.bg.card, color: T.text.primary, fontSize: 10 }} />
                                                <span style={{ position: "absolute", right: 6, top: "50%", transform: "translateY(-50%)", color: T.text.dim, fontSize: 10 }}>%</span>
                                            </div>
                                            <input type="number" inputMode="decimal" pattern="[0-9]*" value={debt.dueDay || ""} onChange={e => { const arr = [...(financialConfig.nonCardDebts || [])]; arr[i] = { ...arr[i], dueDay: parseInt(e.target.value) || 0 }; setFinancialConfig({ ...financialConfig, nonCardDebts: arr }); }}
                                                placeholder="Day" title="Due day of month" style={{ width: "100%", padding: "6px", borderRadius: T.radius.sm, border: `1px solid ${T.border.default}`, background: T.bg.card, color: T.text.primary, fontSize: 10, textAlign: "center" }} />
                                        </div>
                                        <div style={{ fontSize: 8, color: T.text.muted, marginTop: 4, display: "flex", gap: 12 }}>
                                            <span>Balance</span><span>Min Payment</span><span>APR %</span><span>Due Day</span>
                                        </div>
                                    </div>
                                ))}
                        ...
                        < div style={{ marginBottom: 24, display: financeTab === "debts" ? "block" : "none" }}>
                            <h3 style={{ fontSize: 13, fontWeight: 700, marginBottom: 12, color: T.text.primary, borderBottom: `1px solid ${T.border.subtle}`, paddingBottom: 6 }}>Credit Score & Utilization</h3>
                            <p style={{ fontSize: 10, color: T.text.muted, marginBottom: 12, lineHeight: 1.4 }}>Enter your latest score. The AI will factor utilization into debt paydown strategy.</p>
                        ...
                        < div style={{ marginBottom: 24, display: financeTab === "targets" ? "block" : "none" }}>
                            <h3 style={{ fontSize: 13, fontWeight: 700, marginBottom: 12, color: T.text.primary, borderBottom: `1px solid ${T.border.subtle}`, paddingBottom: 6 }}>Targets & Baselines</h3>
                            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 12 }}>
                                <div>
                                ...
                                <div>
                                    ...
                                    <p style={{ fontSize: 11, color: T.text.muted, marginTop: 3, lineHeight: 1.3 }}>Goal: minimum checking balance to maintain.</p>
                                </div>
                            </div>
                                ...
                                <div>
                                    ...
                                    <div style={{ position: "relative" }}>
                                        <input type="number" inputMode="decimal" pattern="[0-9]*" step="1" min="0" max="50" value={financialConfig?.taxBracketPercent || ""} onChange={e => setFinancialConfig({ ...financialConfig, taxBracketPercent: parseFloat(e.target.value) || 0 })}
                                            placeholder="e.g. 22" style={{ width: "100%", padding: "10px 22px 10px 10px", borderRadius: T.radius.md, border: `1px solid ${T.border.default}`, background: T.bg.elevated, color: T.text.primary, fontSize: 12 }} />
                                        <span style={{ position: "absolute", right: 10, top: "50%", transform: "translateY(-50%)", color: T.text.dim, fontSize: 12, fontWeight: 600 }}>%</span>
                                    </div>
                                    <p style={{ fontSize: 11, color: T.text.muted, marginTop: 3, lineHeight: 1.3 }}>Federal bracket (post-tax paychecks already assumed).</p>
                                </div>
                                <div>
                                    ...
                                    <div style={{ position: "relative" }}>
                                        ...
                                        <input type="number" inputMode="decimal" pattern="[0-9]*" step="0.01" value={financialConfig?.minCashFloor || ""} onChange={e => setFinancialConfig({ ...financialConfig, minCashFloor: parseFloat(e.target.value) || 0 })}
                                            placeholder="e.g. 1000" style={{ width: "100%", padding: "10px 10px 10px 22px", borderRadius: T.radius.md, border: `1px solid ${T.border.default}`, background: T.bg.elevated, color: T.text.primary, fontSize: 12 }} />
                                    </div>
                                    <p style={{ fontSize: 11, color: T.text.muted, marginTop: 3, lineHeight: 1.3 }}>Hard floor: AI will never drop total liquid cash below this.</p>
                                </div>
                            </div>
                            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 12 }}>
                                <div>
                                ...
                                <div>
                                    ...
                                    <div style={{ position: "relative" }}>
                                        <input type="number" inputMode="decimal" pattern="[0-9]*" step="0.01" value={financialConfig?.defaultAPR || ""} onChange={e => setFinancialConfig({ ...financialConfig, defaultAPR: parseFloat(e.target.value) || 0 })}
                                            style={{ width: "100%", padding: "10px 22px 10px 10px", borderRadius: T.radius.md, border: `1px solid ${T.border.default}`, background: T.bg.elevated, color: T.text.primary, fontSize: 12 }} />
                                        <span style={{ position: "absolute", right: 10, top: "50%", transform: "translateY(-50%)", color: T.text.dim, fontSize: 13, fontWeight: 600 }}>%</span>
                                    </div>
                                </div>
                                <div>
                                    ...
                                    <div style={{ fontSize: 11, color: T.text.muted, marginTop: 4 }}>Surplus routed to investing if debt APR is below this</div>
                                </div>
                            </div>

                            {/* Tax Withholding */}
                            <h3 style={{ fontSize: 13, fontWeight: 700, marginBottom: 12, marginTop: 16, color: T.text.primary, borderBottom: `1px solid ${T.border.subtle}`, paddingBottom: 6 }}>Tax & Withholding</h3>
                                ...
                                financialConfig?.isContractor && (
                                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                                        ...
                                        <div>
                                            ...
                                            <p style={{ fontSize: 11, color: T.text.muted, marginTop: 4 }}>Due: Apr 15, Jun 15, Sep 15, Jan 15</p>
                                        </div>
                                    </div>
                                )
                            }
                        ...
                        < div style={{ marginBottom: 24, display: financeTab === "assets" ? "block" : "none" }}>
                            <h3 style={{ fontSize: 13, fontWeight: 700, marginBottom: 12, color: T.text.primary, borderBottom: `1px solid ${T.border.subtle}`, paddingBottom: 6 }}>Investments (Defaults)</h3>
                            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                                <div>
                            ...
                            <h3 style={{ fontSize: 13, fontWeight: 700, marginBottom: 12, marginTop: 16, color: T.text.primary, borderBottom: `1px solid ${T.border.subtle}`, paddingBottom: 6 }}>Other Assets (Net Worth)</h3>
                            <p style={{ fontSize: 10, color: T.text.muted, marginBottom: 12, lineHeight: 1.4 }}>Include non-liquid assets in net worth calculations.</p>
                            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 12 }}>
                                <div>
                            ...
                            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                                ...
                                <div>
                                    ...
                                    <input value={financialConfig?.otherAssetsLabel || ""} onChange={e => setFinancialConfig({ ...financialConfig, otherAssetsLabel: e.target.value })}
                                        placeholder="e.g. Crypto, Jewelry" style={{ width: "100%", padding: "10px", borderRadius: T.radius.md, border: `1px solid ${T.border.default}`, background: T.bg.elevated, color: T.text.primary, fontSize: 12 }} />
                                </div>
                            </div>
                            ...
                            <div style={{ marginTop: 14, display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
                                <div>
                                    <span style={{ fontSize: 11, color: T.text.dim, fontFamily: T.font.mono, fontWeight: 600, display: "block" }}>TRACK ROTH CONTRIBUTIONS</span>
                                    <span style={{ fontSize: 10, color: T.text.muted }}>Include YTD + limit in snapshots</span>
                                </div>
                                ...
                                financialConfig?.trackRothContributions && (
                                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginTop: 12 }}>
                                        <div>
                                            ...
                                            <div style={{ fontSize: 11, color: T.text.muted, marginTop: 4 }}>Manual override allowed</div>
                                        </div>
                                        <div>
                                        ...
                                        <div style={{ gridColumn: "1 / span 2", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, background: T.bg.elevated, borderRadius: T.radius.md, padding: "8px 10px", border: `1px solid ${T.border.default}` }}>
                                            <div>
                                                <span style={{ fontSize: 10, color: T.text.dim, fontFamily: T.font.mono, fontWeight: 700, display: "block" }}>AUTO-TRACK YTD (AUDITS)</span>
                                                <span style={{ fontSize: 11, color: T.text.muted }}>Disable to fully control YTD manually</span>
                                            </div>
                            ...
                            <div style={{ marginTop: 18, display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
                                <div>
                                    <span style={{ fontSize: 11, color: T.text.dim, fontFamily: T.font.mono, fontWeight: 600, display: "block" }}>TRACK 401K</span>
                                    <span style={{ fontSize: 10, color: T.text.muted }}>Include balance + YTD + limit</span>
                                </div>
                            ...
                            </div>
                            {
                                financialConfig?.track401k && (
                                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginTop: 12 }}>
                                        ...
                                        {/* ── Employer Match ───────────────────────────── */}
                                        <div style={{ gridColumn: "1 / span 2", paddingTop: 12, borderTop: `1px solid ${T.border.subtle}` }}>
                                            <span style={{ fontSize: 11, color: T.accent.emerald, fontFamily: T.font.mono, fontWeight: 700, display: "block", marginBottom: 8 }}>EMPLOYER MATCH</span>
                                        </div>
                                        <div>
                                            ...
                                            <div style={{ position: "relative" }}>
                                                <input type="number" inputMode="decimal" pattern="[0-9]*" step="1" min="0" max="500" value={financialConfig?.k401EmployerMatchPct ?? ""} onChange={e => setFinancialConfig({ ...financialConfig, k401EmployerMatchPct: parseFloat(e.target.value) || 0 })}
                                                    placeholder="e.g. 100" style={{ width: "100%", padding: "10px 22px 10px 10px", borderRadius: T.radius.md, border: `1px solid ${T.border.default}`, background: T.bg.elevated, color: T.text.primary, fontSize: 12 }} />
                                                <span style={{ position: "absolute", right: 10, top: "50%", transform: "translateY(-50%)", color: T.text.dim, fontSize: 13, fontWeight: 600 }}>%</span>
                                            </div>
                                            <p style={{ fontSize: 11, color: T.text.muted, marginTop: 3 }}>Employer matches X% of contributions</p>
                                        </div>
                                        <div>
                                            ...
                                            <div style={{ position: "relative" }}>
                                                <input type="number" inputMode="decimal" pattern="[0-9]*" step="0.5" min="0" max="100" value={financialConfig?.k401EmployerMatchLimit ?? ""} onChange={e => setFinancialConfig({ ...financialConfig, k401EmployerMatchLimit: parseFloat(e.target.value) || 0 })}
                                                    placeholder="e.g. 6" style={{ width: "100%", padding: "10px 22px 10px 10px", borderRadius: T.radius.md, border: `1px solid ${T.border.default}`, background: T.bg.elevated, color: T.text.primary, fontSize: 12 }} />
                                                <span style={{ position: "absolute", right: 10, top: "50%", transform: "translateY(-50%)", color: T.text.dim, fontSize: 13, fontWeight: 600 }}>%</span>
                                            </div>
                                            <p style={{ fontSize: 11, color: T.text.muted, marginTop: 3 }}>Up to X% of your salary</p>
                                        </div>
                                        <div>
                                            ...
                                            <p style={{ fontSize: 11, color: T.text.muted, marginTop: 3 }}>% of matched funds you own now</p>
                                        </div>
                                        <div>
                                            ...
                                            <div style={{ position: "relative" }}>
                                                <input type="number" inputMode="decimal" pattern="[0-9]*" step="5" min="0" max="100" value={financialConfig?.k401StockPct ?? 90} onChange={e => setFinancialConfig({ ...financialConfig, k401StockPct: parseFloat(e.target.value) || 0 })}
                                                    placeholder="e.g. 90" style={{ width: "100%", padding: "10px 22px 10px 10px", borderRadius: T.radius.md, border: `1px solid ${T.border.default}`, background: T.bg.elevated, color: T.text.primary, fontSize: 12 }} />
                                                <span style={{ position: "absolute", right: 10, top: "50%", transform: "translateY(-50%)", color: T.text.dim, fontSize: 13, fontWeight: 600 }}>%</span>
                                            </div>
                                            <p style={{ fontSize: 11, color: T.text.muted, marginTop: 3 }}>Equity % in your 401k</p>
                                        </div>
                                    </div>
                                )
                            }
                            ...
                            {financialConfig?.enableHoldings && (() => {
                                ...
                                return <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                                    {sections.map(({ key, label, color }) => (
                                        <div key={key} style={{ background: T.bg.elevated, borderRadius: T.radius.md, padding: "10px 12px", border: `1px solid ${T.border.default}` }}>
                                            <span style={{ fontSize: 10, fontFamily: T.font.mono, fontWeight: 700, color: color || T.accent.emerald, display: "block", marginBottom: 6 }}>{label} HOLDINGS</span>
                                            {(holdings[key] || []).map((h, i) => (
                                                <div key={i} style={{ display: "flex", alignItems: "center", gap: 6, padding: "6px 0", borderBottom: i < (holdings[key] || []).length - 1 ? `1px solid ${T.border.subtle}` : "none" }}>
                                                    <span style={{ fontSize: 11, fontWeight: 700, fontFamily: T.font.mono, minWidth: 52, color: T.text.primary }}>{h.symbol.replace("-USD", "")}</span>
                                                    <span style={{ fontSize: 10, color: T.text.secondary, minWidth: 60 }}>{h.shares} {key === "crypto" ? "tokens" : "shares"}</span>
                                                    {marketPrices[h.symbol] && (
                                                        <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", flex: 1 }}>
                                                            <span style={{ fontSize: 12, color: color || T.accent.emerald, fontFamily: T.font.mono, fontWeight: 700 }}>{fmt(marketPrices[h.symbol].price * h.shares)}</span>
                                                            <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                                                                <span style={{ fontSize: 9, color: T.text.muted, fontFamily: T.font.mono }}>@{fmt(marketPrices[h.symbol].price)}</span>
                                                                {marketPrices[h.symbol].changePct != null && (
                                                                    <span style={{ fontSize: 9, fontFamily: T.font.mono, fontWeight: 600, color: marketPrices[h.symbol].changePct >= 0 ? T.status.green : T.status.red }}>
                                                    ...
                                                    <button onClick={() => removeHolding(key, i)} style={{ width: 20, height: 20, border: "none", background: T.status.redDim, color: T.status.red, borderRadius: 4, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14, lineHeight: 1 }}>×</button>
                                                </div>
                                            ))}
                                            <div style={{ display: "flex", gap: 6, marginTop: 6 }}>
                                                {key === "crypto" ? (
                                                    <div style={{ flex: 0.5 }}>
                                                        <SearchableSelect
                                                            ...
                                                            options={[
                                                                ...POPULAR_CRYPTO.map(c => ({ value: c.symbol, label: `${c.symbol.replace('-USD', '')} — ${c.name}` })),
                                                                { value: "__custom__", label: "Custom ticker…" }
                                                            ]}
                                                        />
                                                    </div>
                                                ) : (
                                                    <input value={newHoldingSymbol[key] || ""} onChange={e => setNewHoldingSymbol(p => ({ ...p, [key]: e.target.value }))} placeholder="Ticker"
                                                        style={{ flex: 0.4, padding: "6px 8px", borderRadius: T.radius.sm, border: `1px solid ${T.border.default}`, background: T.bg.base, color: T.text.primary, fontSize: 11, fontFamily: T.font.mono, outline: "none", textTransform: "uppercase" }} />
                                                )}
                                                <input type="number" inputMode="decimal" value={newHoldingShares[key] || ""} onChange={e => setNewHoldingShares(p => ({ ...p, [key]: e.target.value }))} placeholder={key === "crypto" ? "Amount" : "Shares"}
                                                    style={{ flex: 0.3, padding: "6px 8px", borderRadius: T.radius.sm, border: `1px solid ${T.border.default}`, background: T.bg.base, color: T.text.primary, fontSize: 11, fontFamily: T.font.mono, outline: "none" }} />
                                                <button onClick={() => addHolding(key)} style={{ flex: 0.2, padding: "6px", borderRadius: T.radius.sm, border: "none", background: `${(color || T.accent.emerald)}18`, color: color || T.accent.emerald, fontSize: 11, fontWeight: 700, cursor: "pointer" }}>+ Add</button>
                                            </div>
                                        </div>
                                    ))}
                                    <button onClick={refreshPrices} disabled={refreshingPrices} style={{
                                        padding: "10px 14px", borderRadius: T.radius.md, border: `1px solid ${T.accent.emerald}30`,
                                        background: `${T.accent.emerald}10`, color: T.accent.emerald, fontFamily: T.font.mono,
                                        fontSize: 11, fontWeight: 700, cursor: refreshingPrices ? "wait" : "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 6
                                    }}>
                                        {refreshingPrices ? "Refreshing…" : "↻ Refresh Prices"}
                                    </button>
                                    {lastPriceRefresh && <span style={{ fontSize: 10, color: T.text.muted, textAlign: "center", display: "block" }}>
                                        Last refreshed: {new Date(lastPriceRefresh).toLocaleString()}
                                    </span>}
                                </div>;
                            })()}
                        </div>
                        ...
                        < div style={{ marginBottom: 24, display: financeTab === "rules" ? "block" : "none" }}>
