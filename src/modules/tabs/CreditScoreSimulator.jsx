 ...
 *   75%+  = Very Poor
 */
function utilizationScoreContribution(utilPct) {
 ...
 */
function utilizationTier(utilPct) {
...
export default function CreditScoreSimulator({ cards = [], financialConfig = {} }) {
  ...
  const analysis = useMemo(() => {
    ...
    for (const c of activeCards) {
      const bal = Math.max(0, parseFloat(c._plaidBalance ?? c.balance) || 0);
      const lim = Math.max(0, parseFloat(c._plaidLimit ?? c.creditLimit ?? c.limit) || 0);
      totalBalance += bal;
      totalLimit += lim;
      if (lim > 0) {
        cardBreakdown.push({
