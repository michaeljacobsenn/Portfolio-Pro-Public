...
export const WizSelect = ({ value, onChange, options, "aria-label": ariaLabel }) => (
  ...
  >
    {options.map(o => (
      <option key={o.value ?? o} value={o.value ?? o} style={{ background: T.bg.elevated }}>
        {o.label ?? o}
      </option>
    ))}
  </select>
...
// ─── PAGE 1: Import ───────────────────────────────────────────────────────────
export function PageImport({ onNext, toast, onComplete, appleLinkedId, setAppleLinkedId, security, updateSecurity }) {
  ...
  const applyBackup = async backup => {
    ...
    if (Array.isArray(sanitizedPlaid) && sanitizedPlaid.length > 0) {
      ...
      const reconnectCount = sanitizedPlaid.filter(c => !existingIds.has(c.id)).length;
      if (reconnectCount > 0) {
  ...
  const handleSpreadsheet = async file => {
    ...
    try {
      ...
      toast?.error("Spreadsheet import failed: " + e.message);
    }
    setImporting(false);
  };
          ...
          <button
            onClick={async () => {
              try {
                ...
                console.error("[iCloud] Restore error:", e);
                if (toast && !e.message?.toLowerCase().includes("cancel")) toast.error("Apple Sign-In cancelled or failed.");
              }
            }}
            style={{
              width: "100%",
        ...
        <div style={{ marginBottom: 14 }}>
          ...
          >
            <WizSelect
              ...
              onChange={v => updateSecurity("autoBackupInterval", v)}
              options={[
...
export function PageProfile({ data, onChange, onNext, onBack }) {
  return (
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginTop: 16, marginBottom: 24 }}>
        <WizField label="Birth Year" hint="For retirement timelines">
          <WizInput
            ...
            onChange={v => onChange("birthYear", v ? Number(v) : null)}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
        ...
        <WizField label="Deposit Into" hint="Where the funds land">
          <WizSelect
            ...
            onChange={v => onChange("paycheckDepositAccount", v)}
            options={[
