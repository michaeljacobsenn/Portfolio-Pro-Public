...
interface SimDebt {
  ...
  apr: number | string | null | undefined;
  minPayment: number | string | null | undefined;
...
function computeMinPaymentCents(balanceCents: number, aprBps: number) {
...
function triggerHaptic(type: "selection" | "light") {
  void haptic[type]();
}
...
function simulatePayoff(debts: SimDebt[], extraMonthly: number, strategy: "avalanche" | "snowball"): PayoffResult {
...
export default function DebtSimulator({ cards = [], financialConfig }: DebtSimulatorProps) {
  ...
  const debts = useMemo(() => {
    ...
    const nonCardDebts: SimDebt[] = (financialConfig?.nonCardDebts || [])
      .filter((d: NonCardDebt) => Number(d.balance) > 0)
      .map(d => ({
        name: d.name || "Loan",
        balance: d.balance,
        apr: d.apr || 0,
    ...
    <Card animate style={{ position: "relative" }}>
      {showSandbox && (
        <ScenarioSandbox
          ...
          currentAnnualExpenses={(financialConfig?.budgetCategories?.reduce((sum, c) => sum + (c.monthlyTarget || 0), 0) || 0) * 12}
