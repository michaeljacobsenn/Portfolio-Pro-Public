...
interface NegotiationTarget {
  target: string;
  estimatedAnnualSavings?: number | string | null;
  strategy?: string;
}

interface BillNegotiationCardProps {
  cards?: PortfolioCard[];
  financialConfig?: Partial<CatalystCashConfig>;
...
export default function BillNegotiationCard({
  ...
  const validTargets = negotiationTargets.filter(
    (t): t is NegotiationTarget => Boolean(t && t.target && typeof t.target === "string")
  );
  if (validTargets.length === 0) return null;

  const totalPotential = validTargets.reduce((s, o) => s + (parseFloat(String(o.estimatedAnnualSavings ?? 0)) || 0), 0);
