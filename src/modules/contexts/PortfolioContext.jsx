...
export function PortfolioProvider({ children }) {
    ...
    const initPortfolio = async () => {
      try {
        ...
        activeRenewals = activeRenewals.map(r => {
          ...
          if (newDate !== r.nextDue) {
            renewalsChanged = true;
            return { ...r, nextDue: newDate };
          }
          return r;
        });
        ...
        activeCards = activeCards.map(c => {
          ...
          if (newDate !== c.annualFeeDue) {
            cardsChanged = true;
            return { ...c, annualFeeDue: newDate };
          }
          return c;
        });
  ...
  useEffect(() => {
    ...
    fetchMarketPrices(syms)
      .then(p => {
        if (p && Object.keys(p).length > 0) setMarketPrices(p);
      })
      .catch(() => {});
