...
import { getISOWeekNum, getISOWeek } from "../dateHelpers.js";
...
export function AuditProvider({ children }) {
  ...
  // getISOWeekNum, getISOWeek, computeStreak imported from dateHelpers.js
    ...
    async (msg, formData, testMode = false, manualResultText = null) => {
      ...
      try {
        ...
        if (manualResultText) {
          ...
          const [chatSummary, chatHistory] = await Promise.all([db.get("ai-chat-summary"), db.get("ai-chat-history")]);
          ...
          const rawLivePrompt = getSystemPrompt(
            aiProvider || "gemini",
            financialConfig,
            cards,
            promptRenewals,
          ...
          const historyForProvider =
            aiProvider === "gemini"
              ? history.map(m => ({
                ...
                parts: [{ text: scrubber.scrub(m.content) }],
              }))
        ...
        if (!testMode) {
          ...
          try {
            // Compute real streak from audit history using imported helper
            const { computeStreak } = await import("../dateHelpers.js");
            computedStreak = computeStreak(nh);
  ...
  const handleManualImport = useCallback(
    ...
    [navTo, setResultsBackTarget, toast, financialConfig] // Only true dependencies
  );
