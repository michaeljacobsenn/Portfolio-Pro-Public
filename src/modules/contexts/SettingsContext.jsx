...
export function SettingsProvider({ children }) {
  ...
  useEffect(() => {
    const initSettings = async () => {
      try {
        // Run DB gets and permission check concurrently
        const notifPromise = getNotificationPermission().then(status => status === "granted").catch(() => false);

        const [legacyKey, provId, modId, finConf, pr, consent, savedPersona, backupInterval, savedTheme, notifGranted] =
          await Promise.all([
            db.get("api-key"),
            db.get("ai-provider"),
            db.get("ai-model"),
